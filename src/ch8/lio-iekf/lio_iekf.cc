...
void LioIEKF::ProcessMeasurements(const MeasureGroup &meas) {
    LOG(INFO) << "ProcessMeasurements, imu: " << meas.imu_.size() << ", lidar pts: " << meas.lidar_->size();
    measures_ = meas;
...
void LioIEKF::TryInitIMU() {
    ...
    if (imu_init_.InitSuccess()) {
        // 读取初始零偏，设置ESKF
        lh::IESKFD::Options options;
        // 噪声由初始化器估计
        options.gyro_var_ = sqrt(imu_init_.GetGovGyro()[0]);
        options.acce_var_ = sqrt(imu_init_.GetGovGyro()[0]);
...
void LioIEKF::Align() {
    ...
    ieskf_.UpdateUsingCustomObserve(
        [this](const SE3 &input_pose, Mat18d &HTVH, Vec18d &HTVr) { ndt_.ComputeResidualAndJacobians(input_pose, HTVH, HTVr); });
