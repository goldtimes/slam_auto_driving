...
void GinsPreInteg::AddImu(const IMU& imu) {
    if (first_imu_received_ && first_gnss_received_) {
...
void GinsPreInteg::SetOptions(GinsPreInteg::Options options) {
    ...
    options.ba_rw_info_.diagonal() << ba_rw2, ba_rw2, ba_rw2;
    // gnss噪声
    double gp2 = options_.gnss_pose_noise_ * options_.gnss_pose_noise_;
    ...
    options_.gnss_info_.diagonal() << 1.0 / ga2, 1.0 / ga2, 1.0 / ga2, 
                                      1.0 / gp2, 1.0 / gp2, 1.0 / gp2, 
                                      1.0 / gh2, 1.0 / gh2, 1.0 / gh2;
    // clang-format on
...
void GinsPreInteg::Optimize() {
    ...
    v0_bg->setId(3);
    v0_bg->setEstimate(last_frame_->ba_);
