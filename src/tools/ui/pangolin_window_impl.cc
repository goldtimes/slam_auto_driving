...
bool PangolinWindowImpl::Init() {
    ...
    traj_gps_ui_.reset(new UiTrajectory(Vec3f(1.0, 1.0, 51.0 / 255.0)));  // 黄色
...
bool PangolinWindowImpl::UpdateGlobalMap() {
    ...
    for (const auto &cp : cloud_global_map_) {
        ...
        std::shared_ptr<ui::UiCloud> ui_cloud(new UiCloud);
...
bool PangolinWindowImpl::UpdateGps() {
    ...
    // 更新定位轨迹
    traj_gps_ui_->AddPose(gps_pose_);
...
void PangolinWindowImpl::CreateDisplayLayout() {  // define camera render object (for view / scene browsing)
    auto proj_mat_main =
        pangolin::ProjectionMatrix(win_width_, win_width_, cam_focus_, cam_focus_, win_width_ / 2, win_width_ / 2, cam_z_near_, cam_z_far_);
    ...
    pangolin::View &d_cam3d_main =
        pangolin::Display(dis_3d_main_name_).SetBounds(0.0, 1.0, 0.0, 1.0).SetHandler(new pangolin::Handler3D(s_cam_main_));

    pangolin::View &d_cam3d =
        pangolin::Display(dis_3d_name_).SetBounds(0.0, 1.0, 0.0, 0.75).SetLayout(pangolin::LayoutOverlay).AddDisplay(d_cam3d_main);
    ...
    pangolin::Display(dis_main_name_).SetBounds(0.0, 1.0, pangolin::Attach::Pix(menu_width_), 1.0).AddDisplay(d_cam3d).AddDisplay(d_plot);
}
