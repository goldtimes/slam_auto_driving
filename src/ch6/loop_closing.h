...
class LoopClosing {
   public:
    ...
    LoopClosing() { debug_fout_.open("/home/slam_auto/driving/data/ch6/loops.txt"); }
   ...
   private:
    ...
    std::map<size_t, std::shared_ptr<Submap>> submaps_;  // 所有的submap
    // submap 到 多分辨率似然场之间的对应关系
    ...
    inline static constexpr float candidate_distance_th_ = 15.0;  // candidate frame 与submap中心之间的距离
    inline static constexpr int submap_gap_ = 1;                  // 当前scan 与最近submap编号上的差异阈值
