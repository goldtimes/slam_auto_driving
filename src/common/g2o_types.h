...
namespace lh {
/**
 * SO3 + t类型的pose, 存储时伪装为g2o::VertexSE3，供g2o_viewer查看
 */
...
class EdgeGyroRW : public g2o::BaseBinaryEdge<3, Vec3d, VertexGyroBias, VertexGyroBias> {
   public:
    ...
    Eigen::Matrix<double, 6, 6> GetHessian() {
        ...
        J.block<3, 3>(0, 3) = _jacobianOplusXi;
...
/**
 * 加计随机游走 零偏随机游走的边，约束上一个时刻和下一时刻的ba,bg. 上同
 */
...
class EdgeGNSS : public g2o::BaseUnaryEdge<6, SE3, VertexPose> {
   public:
    ...
    void linearizeOplus() override {
        ...
        // dR / dR
        _jacobianOplusXi.block<3, 3>(0, 3) = (_measurement.so3().inverse() * v->estimate().so3()).jr_inv();
        // dp / dp
