...
void LoopClosure::DetectLoopCandidates() {
    ...
    for (auto iter_first = keyframes_.begin(); iter_first != keyframes_.end(); ++iter_first) {
        ...
        for (auto iter_second = iter_first; iter_second != keyframes_.end(); ++iter_second) {
            ...
            if (t2d < range_th) {
                LoopCandidate c(kf_first->id_, kf_second->id_, kf_first->opti_pose_1_.inverse() * kf_second->opti_pose_1_);
...
void LoopClosure::ComputeLoopCandidates() {
    // 执行计算
    std::for_each(std::execution::par_unseq, loop_candiates_.begin(), loop_candiates_.end(), [this](LoopCandidate& c) { ComputeForCandidate(c); });
    // 保存成功的候选
...
void LoopClosure::ComputeForCandidate(LoopCandidate& candidate) {
    LOG(INFO) << "aligining" << candidate.idx1_ << " with " << candidate.idx2_;
    const int submap_idx_range = 40;

    KFPtr kf1 = keyframes_.at(candidate.idx1_);
    KFPtr kf2 = keyframes_.at(candidate.idx2_);

    auto build_submap_func = [this](int given_id, bool build_in_world) -> CloudPtr {
        ...
        for (int idx = -submap_idx_range; idx < submap_idx_range; idx += 4) {
            ...
            pcl::io::loadPCDFile("/home/slam_auto_driving/data/ch9" + std::to_string(id) + ".pcd", *cloud);
            lh::RemoveGround(cloud, 0.1);
    ...
    auto submap_kf1 = build_submap_func(kf1->id_, true);

    CloudPtr submap_kf2(new PointCloudType);
    pcl::io::loadPCDFile("/home/slam_auto_driving/data/ch9" + std::to_string(kf2->id_) + ".pcd", *submap_kf1);
    ...
    candidate.Tij_ = kf1->opti_pose_1_.inverse() * SE3(q, t);
...
void LoopClosure::SaveResults() {
    ...
    std::ofstream fout("/home/slam_auto_driving/data/ch9/loops.txt");
    for (const auto& lc : loop_candiates_) {
