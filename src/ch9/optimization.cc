...
std::string print_info(const std::vector<T>& edges, double th) {
    ...
    if (!chi2.empty()) {
        std::string str = (fmt % chi2.size() % ave_chi2 % chi2[chi2.size() / 2] % chi2[int(chi2.size() * 0.1)] % chi2[int(chi2.size() * 0.9)] %
...
bool Optimization::Init(int stage) {
    ...
    if (!LoadKeyFrames("/home/slam_auto_driving/data/ch9/keyframes.txt", keyframes_)) {
...
void Optimization::Run() {
    ...
    SaveG2O("/home/slam_auto_driving/data/ch9/before.g2o");
...
// 如果gnss不带旋转的情况下,先求出gnss->lidar的变换矩阵
...
void Optimization::AddLidarEdges() {
    const double lidar_pos_noise = 0.01, lidar_ang_noise = 0.1 * math::kDEG2RAD;  // lidar 观测的噪声
...
void Optimization::SaveResults() {
    ...
    // 写入文件
    system("rm /home/slam_auto_driving/data/ch9/keyframes.txt");
    std::ofstream fout(" /home/slam_auto_driving/data/ch9/keyframes.txt");
    for (auto& kfp : keyframes_) {
