...
bool Frontend::Init() {
    ...
    system("rm -rf /home/slam_auto_driving/data/ch9/*.pcd");
    system("rm -rf /home/slam_auto_driving/data/ch9/keyframes.txt");
...
void Frontend::Run() {
    lh::RosbagIO rosbag_io(bag_path_, DatasetType::NCLT);

    // 先提取RTK pose，注意NCLT只有平移部分
    rosbag_io
        .AddAutoRTKHandler([this](GNSSPtr gnss) {
...
// 可以看到这里是完全没有优化的
void Frontend::ExtractKeyFrame(const lh::NavStated& state) {
    if (last_kf_ == nullptr) {
        ...
        FindGPSPose(kf);
        kf->SaveAndUnloadScan("/home/slam_auto_driving/data/ch9/");
