...
void GenSimuData::Gen() {
    ...
    cv::RNG rng;
    gt_pose_ =
        SE3(SO3::exp(Vec3d(rng.gaussian(options_.pose_rot_sigma_), rng.gaussian(options_.pose_rot_sigma_), rng.gaussian(options_.pose_rot_sigma_))),
            Vec3d(rng.gaussian(options_.pose_trans_sigma_), rng.gaussian(options_.pose_trans_sigma_), rng.gaussian(options_.pose_trans_sigma_)));
    // 生成source
